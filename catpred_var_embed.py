...
# os.makedirs('./ProteinCLIP', exist_ok = True)
# os.makedirs('./ProteinCLIP/cold_drug', exist_ok = True)
# os.makedirs('./ProteinCLIP/cold_protein', exist_ok = True)
# os.makedirs('./ProteinCLIP/embedding_random', exist_ok = True)

# for path in Path('./data_ESM2').iterdir():
  
#     if path.name == 'pooled_embedding':
#         continue
...
#                        protein_records_path= path / 'protein_record_kd_ESM2.pdb',
#                        embedding_columns= 'ProteinCLIP_embedding'
#     )
...
#            "--smiles_columns", "Smiles",
#            "--target_columns", "Label",
#            "--protein_records_path", str( path / "protein_record_kd_ESM2.pdb" ),
#            "--epochs", "30",
#            "--batch_size", "16",
#            "--extra_metrics", "mse", "mae", "r2",
#            "--save_dir", f'./ProteinCLIP/{"_".join(str(path.name).split("_")[-2:])}',
...
#     args.task_names = ["km"]                 # display / task name
...
data_root = Path(
    "/sc/arion/projects/DiseaseGeneCell/Huang_lab_project/EnzymeReactProject_environment/catpred_pipeline/reproduce_weight/merged"
)
...
#     if emb_name == 'MUTAPLM' and run_name in ['cold_protein', 'embedding_random'] and kinetic_param == 'km':
#         continue 
...
for emb in embedding.keys():
    ...
    run_name = "_".join(f"{emb}_random")
        
    save_dir = data_root / kinetic_param / emb_name 
    save_dir.mkdir(parents=True, exist_ok=True)

    protein_record_file = data_root / "protein_record_kd_ESM2.pdb"

    if not (data_root / "train.parquet").exists():
        print(f"Missing train.parquet in {path}")
        continue

    if not (data_root / "val.parquet").exists():
        print(f"Missing val.parquet in {path}")
        continue

    if not (data_root / "test.parquet").exists():
        print(f"Missing test.parquet in {path}")
        continue
    ...
    training_data = get_data(
        path=str(data_root / "train.parquet"),
        smiles_columns=["Smiles"],
        target_columns=["Label"],
        skip_invalid_smiles= True,
        protein_records_path=str(protein_record_file),
    ...
    args = TrainArgs().parse_args([
         "--data_path", str(data_root / "train.parquet"),
         "--dataset_type", "regression",
         "--smiles_columns", "Smiles",
         "--target_columns", "Label",
         "--protein_records_path", str(protein_record_file),
         ...
         "--extra_metrics", "mse", "mae", "r2",
         "--save_dir", str(save_dir),
         "--separate_test_path", str(data_root / "test.parquet"),
         "--separate_val_path", str(data_root / "val.parquet"),
         "--ensemble_size", "10",
         "--add_esm_feats",
         "--seq_embed_dim", "48",
         "--seq_self_attn_nheads", "8",
